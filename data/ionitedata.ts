...
import { OMMap, type CustomsMap } from "./omcustoms";
...
export function getRightCustoms(formatId: ID): CustomsMap | null {
	for (const [marker, customs] of Object.entries(OMMap)) {
		if (formatId.includes(marker)) {
			return customs;
		}
	}
	return null;
