...
class RecommendationsInterface {
	...
	async render(rec: Recommendation, suggested = false) {
		...
		if (suggested) {
			...
			buf += `<img src="${Dex.resourcePrefix}sprites/bwicons/441.png" style="margin:-9px 0 -6px -7px;" width="32" height="32" />`;
		...
		if (rec.userData.avatar) {
			...
			buf += `<img style="margin-bottom:-38px;" src="${Dex.resourcePrefix}sprites/trainers${isCustom ? '-custom' : ''}/${isCustom ? rec.userData.avatar.slice(1) : rec.userData.avatar}.png" width="80" height="80" />`;
