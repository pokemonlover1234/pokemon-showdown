	...
	mafia(query, user) {
		...
		if (isPlayer) {
			...
			if (role) {
				...
				buf += `<table><tr><td style="text-align:center;"><img width="75" height="75" src="${Dex.fxPrefix}mafia-${role.image || 'villager'}.png"></td><td style="text-align:left;width:100%"><ul>${role.memo.map(m => `<li>${m}</li>`).join('')}</ul></td></tr></table>`;
				buf += `</details></p>`;
